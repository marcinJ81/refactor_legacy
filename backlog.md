...
| A4 | **Konfiguracja bez pytań wstępnych** — gotowy `refactor-config.json` podstawiony z zewnątrz zastępuje Pytania 0–2. Format już jest, ale orkiestrator nadal pyta. Do rozstrzygnięcia: co wtedy z wymogiem jawnego wyboru użytkownika (`zrodlo` ≠ `domyslne`). **Konflikt odnotowany 2026-09-19:** przy usuwaniu trybu testowego z orkiestratora wycięta została cała możliwość podstawiania konfiguracji z zewnątrz (zasada „Możliwość podstawienia z zewnątrz" w „Plik konfiguracji" oraz punkt „Konfiguracja bez pytań" w „Odłożone do kolejnych iteracji"). Użytkownik: „sekcji nie przywracaj wrzuć w backloga ten info o tym konflikcie żeby potem na to zajrzeć". Wracając do tej pozycji trzeba tę zasadę napisać od nowa, już bez kontekstu testowego. Pytanie T już nie istnieje. | ⏳ | `changes_test.md` (odpowiedzi, „dodatkowy plik json z dedykowaną konfiguracją harnesa"); `v0.2.0/v0.2.2/changes-orchestrator.md` (odpowiedzi 5 i A) | nigdzie — usunięte z `orkiestrator.md` |
...
| E9 | **Hook Etapu 0 kontra tryb `test`** — rozpoznanie Etapu 0 wykonuje hook `SessionStart` zadeklarowany we frontmatterze `etap0.md`, więc uruchamia się także w trybie `test` i wykonuje się naprawdę. Tabela odstępstw trybu `test` mówi tymczasem, że Etap 0 jest mockowany. Użytkownik: „testami na razie się nie przejmujemy, nie wiadomo czy finalnie nie będą musiały być usunięte z tej wersji". Do rozstrzygnięcia: czy mock Etapu 0 zostaje, czy znika na rzecz hooka. | ⏳ | `v0.2.0/v0.2.1/changes-orchestrator.md`, wpis z 2026-09-18 | `refactor-legacy-v0.2.0/etap0.md` (sekcja „Tryb testowy", adnotacja „w budowie"); `orkiestrator.md` → „Tryb testowy" |
